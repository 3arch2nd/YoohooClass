...
// 💡 [수정] 기존 휠 이벤트를 덮어씌워 줍니다! (휠 돌릴 때 슬라이더도 같이 움직이게)
    scrollArea.addEventListener('wheel', (e) => {
        ...
        // ✨ 마우스 휠로 줌할 때, 슬라이더의 게이지 바도 같이 움직이도록 동기화!
        if (zoomSlider) zoomSlider.value = zoom;
...
showToast('서버에 일정을 저장하는 중', true); // 💡 끝에 , true 추가
    ...
    function renderScheduleTable(schedules) {
        ...
        sortedGroups.forEach((group, index) => {
            ...
            // ✨ [핵심 1] 일정이 딱 1개뿐이면? 귀찮게 접고 펴지 말고 그냥 예전처럼 한 줄로 보여줍니다!
            if (group.items.length === 1) {
                const item = group.items[0];
                const tr = document.createElement('tr');
                tr.dataset.room = item.room; // ✨ 삭제 기능을 위해 방 이름을 숨겨둡니다
            ...
            // ✨ [핵심 2] 일정이 2개 이상이라 복잡할 때만 아코디언(접기/펴기) 형태로 묶습니다!
            else {
                ...
                group.items.forEach(item => {
                    ...
                    detailTr.dataset.room = item.room; // ✨ 삭제 기능을 위해 방 이름을 숨겨둡니다
